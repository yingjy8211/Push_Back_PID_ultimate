...
# =============================================================================
# IMU ODOMETRY WITH OFFSET COMPENSATION
# =============================================================================

class IMUOdometry:
    """
    IMU odometry with compensation for sensor offset from center of rotation.
    
    CONFIGURED FOR YOUR ROBOT:
    - Sensor position: 4.0" forward, 0.22" left of center
    - Sensor rotation: 90° CCW (IMU +Y = robot +X, IMU +X = robot -Y)
    
    This compensates for:
    1. Centripetal acceleration (ω² × radius)
    2. Tangential acceleration (α × radius)
    3. Sensor frame rotation
    """
    
    def __init__(self):
        # Position state (inches in world frame)
        self.x = 0.0
        self.y = 0.0
        ...
        # YOUR SENSOR OFFSET (measured from center of rotation)
        self.offset_forward = 4.0    # Inches forward of center
        self.offset_lateral = 0.22   # Inches left of center
        ...
        self.prev_ax = 0.0
        self.prev_ay = 0.0
        self.FILTER_ALPHA = 0.65
        
        # Previous angular velocity for acceleration compensation
        self.prev_omega = 0.0
        
    def calibrate_bias(self):
        """Calibrate IMU bias while robot is stationary."""
    ...
    def update(self, dt, current_heading):
        """
        Update position with offset and rotation compensation.
        ...
        # Read raw acceleration in IMU frame
        accel_imu_x_raw = inertialSensor.acceleration(XAXIS)
        accel_imu_y_raw = inertialSensor.acceleration(YAXIS)
        
        # Remove bias and convert to in/s²
        accel_imu_x = (accel_imu_x_raw - self.accel_bias_x) * G_TO_INCHES_PER_SEC2
        accel_imu_y = (accel_imu_y_raw - self.accel_bias_y) * G_TO_INCHES_PER_SEC2
        
        # Transform from IMU frame to robot frame (90° CCW rotation)
        # IMU +Y → Robot +X (forward)
        # IMU +X → Robot -Y (left)
        accel_robot_forward = accel_imu_y
        accel_robot_lateral = -accel_imu_x
        
        # Get angular velocity for compensation
        # Calculate from heading change (more reliable than gyro API)
        if not hasattr(self, 'prev_heading'):
            self.prev_heading = current_heading
            omega = 0.0
        else:
            # Angular velocity from heading change
            heading_change = current_heading - self.prev_heading
            ...
            omega = heading_change / dt  # rad/s
            self.prev_heading = current_heading
        
        alpha = (omega - self.prev_omega) / dt  # Angular acceleration
        self.prev_omega = omega
        
        # Compensate for rotational acceleration
        # Sensor offset vector in robot frame
        radius = math.sqrt(self.offset_forward**2 + self.offset_lateral**2)
        
        if radius > 0.01:  # Only compensate if sensor is offset
            # Centripetal acceleration (toward center)
            centripetal_mag = omega * omega * radius
            accel_centrip_forward = -centripetal_mag * self.offset_forward / radius
            accel_centrip_lateral = -centripetal_mag * self.offset_lateral / radius
            
            # Tangential acceleration (perpendicular to radius)
            tangential_mag = alpha * radius
            accel_tangent_forward = -tangential_mag * self.offset_lateral / radius
            accel_tangent_lateral = tangential_mag * self.offset_forward / radius
            
            # Remove rotational accelerations from measured acceleration
            accel_robot_forward -= (accel_centrip_forward + accel_tangent_forward)
            accel_robot_lateral -= (accel_centrip_lateral + accel_tangent_lateral)
        
        # Low-pass filter
        accel_robot_forward = self.FILTER_ALPHA * accel_robot_forward + (1 - self.FILTER_ALPHA) * self.prev_ax
        accel_robot_lateral = self.FILTER_ALPHA * accel_robot_lateral + (1 - self.FILTER_ALPHA) * self.prev_ay
        
        self.prev_ax = accel_robot_forward
        self.prev_ay = accel_robot_lateral
        
        # Transform to world frame
        ax_world = (accel_robot_forward * math.cos(current_heading) - 
                    accel_robot_lateral * math.sin(current_heading))
        ay_world = (accel_robot_forward * math.sin(current_heading) + 
                    accel_robot_lateral * math.cos(current_heading))
...
def compensate_wheel_slippage(raw_distance, average_motor_speed):
    ...
    TUNED FOR YOUR ROBOT based on actual measurements:
    - At 70% speed: Measured 8.5% slippage
    - Field: Foam tiles
    - Motors: 540 RPM, 6:1 ratio
    
    These factors are empirically derived from your 4.5" error test.
    ...
    if speed < 40:
        ...
        slippage_factor = 0.97  # ~3% slippage
    elif speed < 75:
        slippage_factor = 0.92  # ~8% slippage (measured at 70%)
    else:
        slippage_factor = 0.88  # ~12% slippage (estimated at high speed)
...
def autonomousPID_optimized(target_distance, target_heading, 
    ...
    # HEADING CONTROL - Solutions 4 & 5 (TUNED for stability with IMU offset)
    KpRotation = Kp_rotation  # Typically 45-55 for stable operation with IMU
    KiRotation = 0.15         # Reduced integral
    KdRotation = 3.0          # Increased damping
    
    # DRIFT RATE COMPENSATION (Reduced for offset sensor - was causing oscillations at 18.0)
    Kp_drift_rate = 5.0  # Much lower with offset compensation
    ...
    while True:
        ...
        if use_imu_odometry:
            ...
            # SENSOR FUSION: Increased IMU weight due to high encoder slippage
            # Your measurement: 8.5% slippage at 70% speed
            # IMU is more accurate for your robot!
            ENCODER_WEIGHT = 0.50  # Reduced from 0.70 (less trust due to slippage)
            IMU_WEIGHT = 0.50      # Increased from 0.30 (more trust)
...
def quick_test():
    """Quick test for initial validation with IMU enabled."""
    print("\n" + "="*70)
    print(" QUICK TEST - 48\" Forward WITH IMU")
    print("="*70)
    print("IMU offset compensation: 4.0\" forward, 0.22\" left, 90° CCW")
    print("")
    ...
    error = autonomousPID_optimized(
        ...
        export_flag=1,
        Kp_forward=4,
        Kp_rotation=45,  # Lower for IMU with offset
        use_imu_odometry=True  # ENABLED with offset compensation!
    )
